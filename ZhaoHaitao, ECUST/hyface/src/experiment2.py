...
"""
实验二: 各频段与灰度图合并组成特征进行识别，第0通道为单灰度图识别
"""
...
def main():
    ...
    steps = [
        ...
        ('reduce_dim', PCA(n_components=int(18*22*2*0.5))),  
    ...
    logf = open(os.path.join(resultdir, 'exp2_log.txt'), 'w+')
    ...
    for i_channels in range(n_channels):
        if i_channels == 0:
            ...
            X_train_i_channels = np.dstack((X_train[:, :, :, 0], X_train[:, :, :, i_channels]))
            X_test_i_channels = np.dstack((X_test[:, :, :, 0], X_test[:, :, :, i_channels]))
    ...
    logf.close()
    # 保存准确率、损失值
    np.save(os.path.join(resultdir, 'exp2_acc_train.npy'),  np.array(acc_train_saved))
    np.save(os.path.join(resultdir, 'exp2_acc_test.npy'),   np.array(acc_test_saved))
    np.save(os.path.join(resultdir, 'exp2_loss_train.npy'), np.array(loss_train_saved))
