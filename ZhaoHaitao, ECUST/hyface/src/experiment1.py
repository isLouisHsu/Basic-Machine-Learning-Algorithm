...
def main():
    ...
    logf = open(os.path.join(resultdir, 'exp1_log.txt'), 'w+')
    ...
    # 保存准确率、损失值
    np.save(os.path.join(resultdir, 'exp1_acc_train.npy'),  np.array(acc_train_saved))
    np.save(os.path.join(resultdir, 'exp1_acc_test.npy'),   np.array(acc_test_saved))
    np.save(os.path.join(resultdir, 'exp1_loss_train.npy'), np.array(loss_train_saved))
